...
def test_if_none_match(admin_user, client, load_json):
    ...
    res = client.get(
        ...
        headers={"If-None-Match": 'W/"graph-places/us-states-version-0"'},
    )
...
def test_delete_graph_requires_auth(admin_user, client, load_json, bearer_auth):
    admin_user
    id = "places/us-states"
    res = client.put(
        f"/graphs/{id}",
...
def test_update_graph(admin_user, client, load_json):
    ...
    assert (
        data["graphs"][f"http://{DEV_SERVER_NAME}/graphs/{id}"]["features"][0]["names"][
            0
        ]["toponym"]
        == "Minnesooooooota"
    )
    ...
    assert len(data["graphs"]) == 1
    assert (
        data["graphs"][f"http://{DEV_SERVER_NAME}/graphs/{id}?version=0"]["features"][
            0
        ]["names"][0]["toponym"]
        == "Minnesota"
    )
    ...
    assert len(data["graphs"]) == 1
    assert (
        data["graphs"][f"http://{DEV_SERVER_NAME}/graphs/{id}?version=1"]["features"][
            0
        ]["names"][0]["toponym"]
        == "Minnesooooooota"
    )
    assert (
        data["graphs"][f"http://{DEV_SERVER_NAME}/graphs/{id}?version=1"][
            "wasRevisionOf"
        ]
...
def test_group_sibling_graphs(admin_user, client, load_json):
    ...
    assert (
        f"http://{DEV_SERVER_NAME}/graphs/places/" == data["@context"]["graphs"]["@id"]
    )
    ...
    assert f"http://{DEV_SERVER_NAME}/graphs/" == data["@context"]["graphs"]["@id"]
    assert (
        res.headers["Content-Disposition"]
