...
def test_submit_patch(active_user, client, load_json):
    ...
    with app.app_context():
        updated_entities = json.loads(
            database.query_db_for_one(
                "SELECT updated_entities FROM patch_request WHERE id = ?", (patch_id,)
            )["updated_entities"]
        )
        assert updated_entities == ["p0trgkv", "p0trgkvwbjd"]
        created_entities = json.loads(
            database.query_db_for_one(
                "SELECT created_entities FROM patch_request WHERE id = ?", (patch_id,)
            )["created_entities"]
        )
        assert created_entities == []
...
@pytest.mark.client_auth_token("this-token-has-normal-permissions")
def test_merge_patch(active_user, admin_user, client, bearer_auth, load_json):
    ...
    with app.app_context():
        ...
        row = database.query_db_for_one(
            "SELECT applied_to, resulted_in FROM patch_request WHERE id=?", (patch_id,)
        )
...
@pytest.mark.client_auth_token("this-token-has-normal-permissions")
def test_reject_patch(active_user, admin_user, client, bearer_auth, load_json):
    active_user, admin_user
    res = client.patch("/d/", json=load_json("test-patch-adds-items.json"))
    patch_id = int(res.headers["Location"].split("/")[-2])
    patch_url = urlparse(res.headers["Location"]).path
    res = client.post(
        patch_url + "reject", auth=bearer_auth("this-token-has-admin-permissions")
    )
    assert res.status_code == httpx.codes.NO_CONTENT
    with app.app_context():
        row = database.query_db_for_one(
            "SELECT open, merged FROM patch_request WHERE id=?", (patch_id,)
        )
...
def test_comment_on_patch(active_user, client, load_json):
    ...
    with app.app_context():
        row = database.query_db_for_one(
            "SELECT * FROM patch_request_comment WHERE patch_request_id=?", (patch_id,)
        )
...
def test_versioning(client, submit_and_merge_patch):
    ...
    for version in range(1, 4):
        ...
        assert ctx[0] == f"http://{DEV_SERVER_NAME}/c?version={version}"
...
def test_remove_period(client, submit_and_merge_patch):
    ...
    with app.app_context():
        ...
        assert removed_entities == {"p0trgkvwbjd"}
