...
comment_template = Template(
    '<p><span>$commenter $posted_at</span><br>$message</p>'
)
...
def get_content(what, who, submitter, review_patch_url, comments):
    content = {
        'submitted':
        '<p>$who submitted a proposed change:</p>',
        'updated':
        '<p>$who updated a change initially proposed by $submitter:</p>',
        'merged':
        '<p>$who merged a change initially proposed by $submitter:</p>',
        'rejected':
    ...
    content += '<div>'
    content += ''.join([
        comment_template.substitute(comment) for comment in comments
    ])
    content += '</div>'

    return linkify(Template(content).substitute({
        'who': to_link(who),
...
def generate_activity_feed():
    ...
    for row in reversed(recent_activity):
        ...
        review_patch_url = build_client_url(
            page='review-patch',
            patchURL=url_for('patchrequest', id=row['id'])[1:]
        )
        roles = get_roles(row)
