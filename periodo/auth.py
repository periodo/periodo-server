...
def load_identity_from_authorization_header():
    ...
    if not match:
        app.logger.debug("failed to load bearer token from authorization header")
        return UnauthenticatedIdentity("invalid_token", "The access token is malformed")
...
def _get_identity(b64token):
    ...
    if not rows:
        return UnauthenticatedIdentity("invalid_token", "The access token is invalid")
    if rows[0]["token_expired"]:
        return UnauthenticatedIdentity("invalid_token", "The access token expired")
